...
* `kms_key_id` - (Optional) The ARN of the AWS KMS key to use when creating the encrypted volume. **Note:** Despite the parameter name, only the full key ARN is accepted—not the key ID or alias. `encrypted` must be set to `true` when this is set.
